...
function toneFor(action: string): 'mint' | 'clay' | 'amber' | 'ink3' {
  if (/delivered|passed|rewarded|unsuspend/.test(action)) return 'mint';
  if (/fail|suspend|cancel|force/.test(action)) return 'clay';
  if (/admin\./.test(action)) return 'amber';
  return 'ink3';
}
                ...
                <Card key={r.id} padded={false} style={{ padding: 12 }}>
                  <Row style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <View style={{ flex: 1, gap: 2 }}>
                      ...
                      <Body size={11} tone="ink3" numberOfLines={1}>
                        {r.entityType ?? '—'}
                      ...
                      {r.metadata && Object.keys(r.metadata).length > 0 && (
                        ...
                        >
                          <Body size={10.5} tone="ink3">
                            {JSON.stringify(r.metadata)}
                          </Body>
                        </View>
                      )}
                    </View>
                    <Body size={10.5} tone="ink3" style={{ marginLeft: 8 }}>
