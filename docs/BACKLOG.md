...
| 30 | **Staff invitation links point at the customer web app.** The email sends people to `FRONTEND_ORIGIN/auth/reset-password`, which is the customer site — so a compliance analyst claims their back-office account on the page customers use. It works, and it is odd. The claim page belongs in the panel, with the staff email pointing there instead. Not urgent while `FRONTEND_ORIGIN` is a localhost dev server, but it should not be how the company first meets the product. | ⬜ |
