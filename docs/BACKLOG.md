...
| 31 | **The mobile app has no screen for entering a code.** It calls `/auth/forgot-password` and `/auth/resend-verification`, then tells the customer to check their email — and the email used to carry a link to the web app. The web app is retired and the emails now carry six digits, so both flows currently end nowhere. Needs two screens: reset password (email + code + new password → `POST /auth/reset-password`) and verify email (email + code → `POST /auth/verify-email`, which moved from GET). The back office already has the equivalent in `admin/src/routes/ClaimAccount.tsx` — same contract, same wording, worth copying rather than reinventing. **This is the last thing standing between a customer and a locked account**, so it goes before #20. | ⬜ |
